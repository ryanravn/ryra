...
test("paperless-ngx login page loads and has SSO option", async ({ browser }) => {
  ...
  // Paperless-ngx shows a login page with an SSO link/button
  const ssoLink = page.locator("a:has-text('SSO'), button:has-text('SSO'), a:has-text('authelia')");
  await expect(ssoLink).toBeVisible({ timeout: 15_000 });
  await context.close();
});
