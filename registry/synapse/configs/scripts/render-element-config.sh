...
## Without --domain, Element is served at http://127.0.0.1:$RYRA_PORT_HTTP
## and that's also where the browser will call /_matrix (same origin via
## element-nginx.conf). With --domain, SERVICE_EXTERNAL_URL overrides.
set -euo pipefail
...
# set to the --domain (https://…) when present; otherwise fall back to the
# loopback http URL on the published port.
export ELEMENT_BASE_URL="${SERVICE_EXTERNAL_URL:-http://127.0.0.1:${RYRA_PORT_HTTP}}"
