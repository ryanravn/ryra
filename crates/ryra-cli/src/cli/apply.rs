...
async fn execute(step: &Step) -> Result<()> {
    match step {
        ...
        Step::TailscaleServe {
            ...
            // Sudo policy:
            //   1. Try `sudo -n` (cached/passwordless). 5s timeout — `-n`
            //      either succeeds instantly or fails instantly.
            //   2. If that fails AND we have a TTY, fall through to
            //      interactive `sudo` which prompts for a password. 60s
            //      timeout to accommodate password entry plus the
            //      `tailscale serve` call (which can hang ~9 min if Serve
            //      isn't enabled at the tailnet ACL level — bail fast).
            //   3. Otherwise (no TTY, or interactive sudo failed/timed
            //      out) print the manual command. Sudo is opt-in.
            use std::io::IsTerminal;
            use std::time::Duration;
            let target = format!("http://127.0.0.1:{target_port}");
            let https_arg = format!("--https={https_port}");
            let cached_attempt = run_with_timeout(
                "sudo",
                &["-n", "tailscale", "serve", "--bg", &https_arg, &target],
                Duration::from_secs(5),
            );
            let outcome = match cached_attempt {
                // Only re-try with interactive sudo when sudo itself refused.
                // TimedOut means tailscale was stuck (Serve disabled) — re-running
                // would just hang another 60s, so pass that through to the
                // helpful "enable Serve in your ACL" hint.
                TimeoutResult::Failed if std::io::stdin().is_terminal() => {
                    println!();
                    println!(
                    ...
                    run_with_timeout(
                        "sudo",
                        &["tailscale", "serve", "--bg", &https_arg, &target],
            ...
            match outcome {
                // tailscale itself prints "Serve started and running in the
                // background" plus the off-command on success, so we don't
                // add our own line — would just duplicate the same info.
                TimeoutResult::Success => {}
                TimeoutResult::Failed => {
                    println!();
                    println!("  To expose this service on your tailnet, run (requires sudo):");
                    println!("    sudo tailscale serve --bg --https={https_port} {target}");
                }
                TimeoutResult::TimedOut => {
                    println!();
                    println!(
                        "  `tailscale serve` didn't respond — Serve is likely not enabled"
                    );
                    println!("    on your tailnet. Enable it at");
                    println!(
                        "    https://login.tailscale.com/admin/acls (look for `serve`/`funnel`),"
                    );
                    println!("    then run:");
                    println!("    sudo tailscale serve --bg --https={https_port} {target}");
                }
...
/// Outcome of a bounded-time command run.
enum TimeoutResult {
    /// Process exited with a successful status code.
    Success,
    /// Process exited with a non-zero status (or failed to spawn).
    Failed,
    /// Process didn't exit within the timeout and was killed.
    TimedOut,
}

/// Run `cmd args...`, killing the child if it doesn't finish within
/// `timeout`. No std crate provides this; we implement it with the
/// try_wait + sleep pattern (10ms poll cadence is plenty — sub-second
/// commands get noticed quickly, and we're not optimising for sub-ms
/// turnaround on the success path).
///
/// Stdio is inherited so interactive `sudo` can read a password from
/// the user's TTY. Non-interactive callers should pass `-n` (or similar
/// flags) to keep the child from reading stdin.
fn run_with_timeout(cmd: &str, args: &[&str], timeout: std::time::Duration) -> TimeoutResult {
    ...
    let mut child = match Command::new(cmd).args(args).spawn() {
        Ok(c) => c,
        // Spawn failure (e.g. sudo not on PATH) is an honest failure, not
        // a timeout — let the caller print the manual fallback.
        Err(_) => return TimeoutResult::Failed,
    };
