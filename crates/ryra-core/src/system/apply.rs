...
async fn execute(step: &Step) -> Result<()> {
    match step {
        Step::WriteFile(file) => {
            ...
            crate::system::atomic_write::atomic_write(&file.path, file.content.as_bytes(), mode)
...
mod tailscale_services {
    use crate::config::schema::{HOST_TAG, SERVICE_TAG, TailscaleConfig};
    use anyhow::{Context, Result, bail};
    use std::process::{Command, Stdio};
    ...
    fn token() -> Result<TailscaleConfig> {
        ...
        let cfg = crate::config::load_or_default(&paths.config_file)?;
    ...
    pub fn ensure_setup() -> Result<()> {
        ...
        let node_dns = crate::system::tailscale::self_dns_name()
        ...
        let tailnet = crate::system::tailscale::tailnet_suffix(&node_dns);
        if tailnet.is_some() && ts.tailnet != tailnet {
            let paths = crate::config::ConfigPaths::resolve()?;
            let mut cfg = crate::config::load_or_default(&paths.config_file)?;
            if let Some(t) = cfg.tailscale.as_mut() {
                t.tailnet = tailnet;
                crate::config::save_config(&paths.config_file, &cfg)?;
            }
        }
    ...
    pub fn enable(svc_name: &str, ports: &[crate::TailscalePort]) -> Result<()> {
        let ts = token()?;
        let key = &ts.admin_api_key;
