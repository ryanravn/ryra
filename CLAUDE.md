...
4. If the service sends mail, add an `smtp` test using the `mail` step to assert inbucket delivery (see `registry/forgejo/test.toml` or `ryra test ls -v forgejo-smtp`).
