...
5. **Caddy** (optional) is configured as a reverse proxy with automatic HTTPS when you pass `--url`
6. **Authelia** (optional) issues OIDC clients when you pass `--auth`
...
Run `ryra search` to see the full list with install status. The bundled registry includes:
...
- **Not a PaaS**: it's a CLI tool that generates systemd units, not a managed platform.
